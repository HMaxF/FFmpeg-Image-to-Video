...
# --- Duration and FPS ---
DUR=4 # longer is slower and smoother zoom effect.
...
zoom_end=1.1 # 4K (16:9) height is too narrow for my photos that are 2:3, so use smaller zoom_end
zoom_speed=$(printf "%.3f" $(echo "($zoom_end - $zoom_start)/$total_frames" | bc -l)) # variable zoom speed ==> 0.2 / 120 frames ==> 0.0016666666666666667
zoom_speed=0.002 # increasing to 0.003 is the solution to jittery or shaky zoom effect
...
for file_data in "${images_file_data[@]}"; do
    ...
    if [[ $use_various_image_ratios -eq 0 ]]; then
        ...
        # NOTE: without 'scale', operation is SLOWER but zooming is smoother and a little bit larger filesize.
        ...
        fc+="scale=$video_width:$video_height:force_original_aspect_ratio=decrease" # scale down IF too large .. also make operation FASTER but jittery zoom effect
    ...
    #fc+=",vignette=$vignette" # always vignette for immersive zoom effect
    # if [[ $use_various_image_ratios -eq 0 ]]; then
...
cli=(ffmpeg -hide_banner -loglevel error 
    ...
    -s $video_resolution 
    -pix_fmt yuv420p 
    # -preset ultrafast # for faster encoding but larger file size (no compression)
    -preset fast # slower encoding but better quality and smaller file size

    # 18–20: very high quality, bigger file
    # 23: default, good balance
    # 26–28: lower quality, much smaller size is better quality but larger file size
    -crf 24
